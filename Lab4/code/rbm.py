...
class RestrictedBoltzmannMachine():
    ...
    def cd1(self, visible_trainset, n_iterations=10000):
        ...
        for it in range(n_iterations):
            ...
            errors[it] = np.sum((visible_trainset - v_k) ** 2) / n_samples
            print("iteration=%7d recon_loss=%4.4f" % (it, errors[it]))
    ...
    def get_v_given_h(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            samples = np.concatenate((v_label, v_img), axis=1)
        else:
    ...
    def get_v_given_h_dir(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            raise ValueError('the RBM is a part of a DBN and is at the top')
    ...
    def update_generate_params(self, inps, trgs, preds):
        ...
        self.delta_weight_h_to_v += self.learning_rate*np.dot(inps.transpose(), (trgs-preds))/self.batch_size
        self.delta_bias_v += self.learning_rate*np.sum(trgs-preds, axis=0)/self.batch_size
    ...
    def update_recognize_params(self, inps, trgs, preds):
        ...
        self.delta_weight_v_to_h += self.learning_rate*np.dot(inps.transpose(), (trgs-preds))/self.batch_size
        self.delta_bias_h += self.learning_rate*np.sum(trgs-preds, axis=0)/self.batch_size
